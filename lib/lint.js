...
module.exports = function (options) {
    ...
    const edition = options.edition = options.edition || 'latest';
    if (typeof edition === 'function') delete options.edition;
    ...
    if (typeof edition === 'function') lintStream.JSlint = edition;

    // map the incoming vinyl object to the object
    // expected by the lint stream
    const stream = map(function (data, next) {
    ...
    return map((source, next) => stream.write({
        source: source,
        done: (err) => {
            if (err) next(err);
            else next(null, source);
        }
    }));
};
